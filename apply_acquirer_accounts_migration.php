...
try {
    ...
    CREATE TABLE IF NOT EXISTS acquirer_accounts (
      ...
      merchant_id VARCHAR(255) COMMENT 'Used as x-withdraw-key for cashout operations',
    ...
    if ($result['count'] == 0) {
        ...
        VALUES
          (1, 'Conta Principal 1', 'ACC-000001', 'client_id_main_1', 'secret_main_1', 'withdraw_key_001', 1, 1),
          (1, 'Conta Principal 2', 'ACC-000002', 'client_id_main_2', 'secret_main_2', 'withdraw_key_002', 1, 0),
          (2, 'Conta Backup 1', 'ACC-000003', 'client_id_backup_1', 'secret_backup_1', 'withdraw_key_003', 1, 1)
        ";
        $db->exec($sql);
        ...
        // Update existing accounts with account_identifier if missing
        echo "Verificando account_identifier nas contas existentes...\n";
        $sql = "UPDATE acquirer_accounts
                SET account_identifier = CONCAT('ACC-', LPAD(id, 6, '0'))
                WHERE account_identifier IS NULL";
        $affected = $db->exec($sql);
        if ($affected > 0) {
            echo "✓ {$affected} contas atualizadas com account_identifier\n";
        }
    }
